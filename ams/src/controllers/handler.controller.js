...
const handlePost = async (req, res) => {
  ...
  try {
    ...
    if (found) {
      console.log(`[${timestamp}] Meter no ${meterNo} found — AMS is available`);
      return res.status(200).send('I am AMS, and I am available. I can handle meter no ' + meterNo);
    } else {
      console.log(`[${timestamp}] Meter no ${meterNo} NOT found — AMS cannot handle it`);
      return res.status(503).send('AMS cannot handle meter no ' + meterNo);
    }
  } catch (err) {
